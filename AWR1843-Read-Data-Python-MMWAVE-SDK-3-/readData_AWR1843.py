...
def update():
    ...
    if dataOk and len(detObj["x"])>0:
        ...
        y = detObj["y"]
        #initialisation
        detObj['distance'] = np.array([])
        detObj['velocity'] = np.array([])

        for line in range(0, int(len(detObj['x']))):
            #we compute the distance using cartesian system
            distance = round(math.sqrt(math.pow(detObj['x'][line],2) + math.pow(detObj['y'][line],2) + math.pow(detObj['z'][line],2)))
            #we save the distance in the array
            detObj['distance'] = np.append(detObj['distance'], distance)

        ax = sns.regplot(data=detObj, x='x', y='distance', fit_reg=False, marker="o", color="red")

        ax.set_xlim(-5,5)
        #distance is always positive
        ax.set_ylim(0,5)

        #for each object, we create a graphical point that we can display on a graphical window
        for line in range (0, len(detObj['x'])):
            plt.text(detObj['x'][line]+0.4, detobj['distance'][line], str(detObj['distance'][line])+'m',
